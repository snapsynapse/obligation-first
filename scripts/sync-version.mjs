...
function locations(f) {
  ...
  return [
    ...
    ["docs/llms.txt", /support\. v\d+\.\d+\.\d+(?:-draft)? is implemented locally across/, `support. ${f.vfull} is implemented locally across`],
    ["docs/llms-full.txt", /gist\. v\d+\.\d+\.\d+(?:-draft)? is implemented locally pending/, `gist. ${f.vfull} is implemented locally pending`],
    ["README.md", /spec-v\d+\.\d+\.\d+(?:--draft)?-orange/, `spec-${f.badge}-orange`],
    ["README.md", /The v\d+\.\d+\.\d+(?:-draft)? implementation is a local release candidate/, `The ${f.vfull} implementation is a local release candidate`],
    ["README.md", /^v\d+\.\d+\.\d+(?:-draft)? is implemented locally across/m, `${f.vfull} is implemented locally across`],
    ...
    ["docs/index.html", /current draft \(v\d+\.\d+\)/g, `current draft (${f.vmm})`],
    ["docs/index.html", /projections using v\d+\.\d+ vocabulary/g, `projections using ${f.vmm} vocabulary`],
    ["docs/index.html", /locally validated v\d+\.\d+ projections/g, `locally validated ${f.vmm} projections`],
