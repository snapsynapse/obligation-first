...
function locations(f) {
  ...
  return [
    ...
    ["docs/llms.txt", /(support\. )v\d+\.\d+\.\d+(?:-draft)?/, `$1${f.vfull}`],
    ["docs/llms-full.txt", /(gist\. )v\d+\.\d+\.\d+(?:-draft)?/, `$1${f.vfull}`],
    ["README.md", /spec-v\d+\.\d+\.\d+(?:--draft)?-orange/, `spec-${f.badge}-orange`],
    ["README.md", /(The )v\d+\.\d+\.\d+(?:-draft)?(?= release is live)/, `$1${f.vfull}`],
    ["README.md", /^v\d+\.\d+\.\d+(?:-draft)?(?= is the current release\.)/m, `${f.vfull}`],
    ...
    ["docs/index.html", /live v\d+\.\d+\.\d+(?:-draft)?(?: release)? of Obligation-First/, `live ${f.vfull} release of Obligation-First`],
    ...
    ["docs/index.html", /((?:locally validated|publish(?:ed)?) )v\d+\.\d+(?= projections)/g, `$1${f.vmm}`],
    ["docs/v1/index.html", /(currently on the )v\d+\.\d+\.\d+(?:-draft)?(?= release line)/, `$1${f.vfull}`],
