...
export async function outputs() {
  ...
  const html = `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Obligation-First vocabulary reference</title><meta name="description" content="Documentation for existing Obligation-First vocabulary IRIs and their JSON-LD context aliases; permanent redirect contribution pending."><link rel="canonical" href="https://obligationfirst.org/v1/vocabulary/"><style>body{font:18px/1.6 system-ui;max-width:60rem;margin:auto;padding:2rem;color:#172033;background:#fff}a{color:#1649a5}section{border-top:1px solid #ccc;padding:1rem 0}code{overflow-wrap:anywhere}</style></head><body><main><a href="/v1/">Namespace index</a><h1>Vocabulary reference</h1><p>These documentation targets describe the names already mapped by the JSON-LD context. They add no vocabulary or legal semantics. The proposed w3id redirect contribution has not been submitted; permanent-IRI resolution remains pending.</p><p>Consult the <a href="/v1/context.jsonld">context</a>, <a href="/v1/schema/">schemas</a> and <a href="https://github.com/snapsynapse/obligation-first/blob/main/PROTOCOL.md">protocol</a> for definitions and constraints. JSON aliases may differ from expanded IRI suffixes. A documentation redirect is not a machine-readable ontology or proof of applicability.</p>${terms.map(t => `<section id="${t.term}"><h2>${t.term}</h2><p>Vocabulary IRI: <code>${t.iri}</code></p><p>Context aliases: ${t.aliases.map(a => `<code>${esc(a)}</code>`).join(', ')}</p></section>`).join('\n')}</main></body></html>\n`;
