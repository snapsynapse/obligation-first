...
import { implementationSummary as producerImplementationSummary, STATUS_SURFACES } from './sync-version.mjs';

export const STATUS_FILES = [
  'reference/implementation-status.json', 'docs/evaluation-status.json',
  'README.md', 'docs/index.html', 'docs/v1/index.html', 'docs/llms.txt', 'docs/llms-full.txt',
  'package.json', 'docs/agents.json',
...
export const STATUS_SURFACE_INVENTORY = Object.freeze([
  ...STATUS_SURFACES.map(([rel, render]) => ({
    rel,
    wrapper: summary => render(summary).trim(),
    discovery: rel === 'docs/v1/index.html' ? '/evaluation-status.json' : 'https://obligationfirst.org/evaluation-status.json',
  })),
  { rel: 'docs/agents.json', discovery: 'https://obligationfirst.org/evaluation-status.json', machine: true },
]);
export const implementationSummary = producerImplementationSummary;
export function implementationStatusFailures(files, manifest) {
  ...
  for (const surface of STATUS_SURFACE_INVENTORY) {
    const text = files[surface.rel];
    if (surface.machine) {
      if (agents.endpoints?.evaluation_status !== surface.discovery) failures.push(`OF-STATUS-DISCOVERY: ${surface.rel} omits machine status`);
      continue;
    }
    const matches = [...text.matchAll(/<!-- implementation-status:start -->([\s\S]*?)<!-- implementation-status:end -->/g)];
    const expected = surface.wrapper(summary);
    if (matches.length !== 1 || matches[0][1].trim() !== expected) failures.push(`OF-STATUS-PROSE: ${surface.rel} scope summary differs`);
    if (!text.includes(surface.discovery)) failures.push(`OF-STATUS-DISCOVERY: ${surface.rel} omits machine status`);
  }
